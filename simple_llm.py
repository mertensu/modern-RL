...
class SimpleDecoderLLM(nn.Module):
    def __init__(self, tokenizer, d_model=64, nhead=4, num_layers=2):
        ...
        # Output heads
        self.action_head = nn.Linear(d_model, tokenizer.vocab_size - 2)  # exclude <bos> and <pad>
    ...
    def sample_actions(self, num_sequences=1, n_actions=None, temperature=1.0):
        ...
        # Initialize
        model_input_sequences = [[self.tokenizer.bos_token_id] for _ in range(num_sequences)]
        ...
        for t in range(n_actions):
            ...
            attention_mask = torch.zeros(num_sequences, len(model_input_sequences[0]), device=device)
...
if __name__ == "__main__":
    ...
    # Decode sequences
    decoded_sequences = tokenizer.decode(samples['sequences'])
