...
from simple_llm import PolicyModel, SimpleTokenizer
import gymnasium as gym
from torch.distributions import Categorical
import matplotlib.pyplot as plt
from typing import List, Dict, Tuple
from vis import visualize_path
...
class PPO:
    ...
    def align_samples_with_episodes(self, samples: Dict, results: List[Dict]) -> None:
        ...
        # Convert to tensors with better variable naming
    ...
    def construct_states_from_episodes(
    ...
    def pad_sequences(
        self, sequences: List[List[int]], padding_value: int = 0
        ...
        padded_sequences = torch.tensor(
            [seq + [padding_value] * (max_len - len(seq)) for seq in sequences],
            device=self.device,
        )
    ...
    def compute_advantages_and_returns(self) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        padded_advantages, attention_mask = self.pad_sequences(
            advantages_list, padding_value=0.0
        )
        ...
        padded_returns, _ = self.pad_sequences(returns_list, padding_value=0.0)
    ...
    def compute_losses(
        ...
        padded_sequences, attention_mask = self.pad_sequences(
            self.states, padding_value=self.model.tokenizer.pad_token_id
        )

        # Forward pass with attention mask
        action_logits, values = self.model(padded_sequences, attention_mask)
        ...
        surr2 = (
            torch.clamp(ratio, 1 - self.epsilon, 1 + self.epsilon) * masked_advantages
        )
    ...
    def update(self, num_epochs: int):
